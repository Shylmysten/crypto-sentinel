...
const fetchUserData = async (apiKey) => {
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}${apiKey}`, {requestOptions});
...
export const getDashboard = async (apiKey) => {
  ...
  return {
    ...
    poolStats: {
      ...
      minPayout: 100, // PowerPool DOGE minimum payout (typical)
      payoutUnit: currency,
      fee: 1.5, // PowerPool typical fee
    },
  };
};
...
export const getWorkers = async (apiKey) => {
  ...
  if (userData.workers) {
    ...
    const algorithms = [...Object.keys(userData.workers)];


    
    for (const id of algorithms) {
      if (userData.workers[id] && Array.isArray(userData.workers[id])) {
        userData.workers[id].forEach(worker => {
